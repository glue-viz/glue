...
def test_edit_subset_mode_toolbar():
    ...
    assert combo_labels(subset_combo) == ('Subset 2', ['Subset 1', 'Subset 2', 'None'])
    assert mode_label.text() == 'Mode:'

    edit.edit_subset = [sg1, sg2]

    assert combo_labels(subset_combo) == ('Multiple subsets', ['Multiple subsets', 'Subset 1', 'Subset 2', 'None'])
    assert mode_label.text() == 'Mode:'

    edit.edit_subset = [sg1]

    assert combo_labels(subset_combo) == ('Subset 1', ['Subset 1', 'Subset 2', 'None'])
    assert mode_label.text() == 'Mode:'
