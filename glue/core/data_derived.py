...
class SlicedData(BaseCartesianData, HubListener):
    """
    A dataset where some dimensions have been sliced.

    The slices can be modified after the data has been created using the
    ``slices`` property.
    ...
    original_data : `~glue.core.data.BaseCartesianData`
        The original data to reduce the dimension of
    slices : tuple of `int` or `None`
        The slices to apply to the data, or `None` if a dimension should be
        preserved. This tuple should have as many elements as there are
        dimensions in ``original_data``.
    """

    def __init__(self, original_data, slices):

        super(SlicedData, self).__init__()

        if len(slices) != original_data.ndim:
            raise ValueError("The 'slices' tuple should have length {0}"
                             .format(original_data.ndim))
        ...
        if hasattr(original_data, 'coords'):
            if original_data.coords is None:
                ...
                slices_0 = [slice(None) if idx == 0 else idx for idx in slices]
                self._coords = SlicedLowLevelWCS(original_data.coords, slices_0)
    ...
    def slices(self, value):
        ...
        # Compute a subset state that represents the indexing - this is used
        # for compute_statistic and compute_histogram
        slice_labels = [slice(x) if x == 0 else x for x in self._slices]
        self._slices_subset_state = SliceSubsetState(self._original_data, slice_labels)

        # Construct a list of original pixel component IDs
        self._original_pixel_cids = []
        for idim in range(self._original_data.ndim):
            if self._slices[idim] is None:
                self._original_pixel_cids.append(self._original_data.pixel_component_ids[idim])
        ...
        # Tell glue that the data has changed
        if has_changed and self.hub is not None:
            msg = NumericalDataChangedMessage(self)
            self.hub.broadcast(msg)

    @property
    def label(self):
        slice = '(' + ','.join([':' if x == 0 else str(x) for x in self._slices]) + ')'
        return self._original_data.label + slice
    ...
    def _to_original_view(self, view):
        if view is None:
            view = [slice(None)] * self.ndim
        original_view = list(self.slices)
        idim_reduced = 0
        for idim in range(self._original_data.ndim):
            if original_view[idim] is None:
                if view is None:
    ...
    def register_to_hub(self, hub):
        hub.subscribe(self, NumericalDataChangedMessage,
                      handler=self._check_for_original_changes)
        super(SlicedData, self).register_to_hub(hub)
    ...
    def compute_histogram(self, *args, **kwargs):
        if kwargs.get('subset_state') is None:
            kwargs['subset_state'] = self._slices_subset_state
        else:
            kwargs['subset_state'] &= self._slices_subset_state
        return self._original_data.compute_histogram(*args, **kwargs)
