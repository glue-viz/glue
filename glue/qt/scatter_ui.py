...
class ScatterUI(QMainWindow, glue.ScatterClient):
    def __init__(self, data=None, parent=None):
        ...
        glue.ScatterClient.__init__(self, data=data, figure=self.fig)
    ...
    def notify_layer_change(self, old, new):
        ...
        if self.toolbar is not None:
            ...
            self.toolbar.set_roi_enabled(isinstance(new, glue.subset.RoiSubset))
    ...
    def _add_tree_item(self, item):
        tree = self.tree['root']
        if isinstance(item, glue.Subset):
            d = item.data
            parent = self.tree[d]
            label = item.label
            if label is None:
            ...
            parent = self.tree[d]
        elif isinstance(item, glue.Data):
            label = item.label
            if label is None:
...
if __name__=="__main__":
    ...
    hub = glue.Hub(subset_client, scatter_client, data, data2, s, s2)
