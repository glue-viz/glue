...
cube to expose the values along it, and these helpers wire the slices up
so a path-slice viewer can render them together.

The hookup has two parts:

1. **Per-slice "self" links** between each slice's non-path pixel CIDs and
   ...
   so the chain
   ``slice_a.non_path -> slice_a.original_data.non_sliced -> ... -> slice_b.original_data.non_sliced -> slice_b.non_path``
   is then walkable.

2. **Pairwise path links** between every pair of slices: a
   :class:`PathRelativeLink` that maps path-axis pixel ``k`` in one slice
...
class PathRelativeLink(ComponentLink):
    ...
    The forward mapping is ``k_to = k_from * len(slice_to.x) / len(slice_from.x)``.
    Path lengths are read on each call, so the link automatically reflects
    the current state of both slices even after :meth:`PathSlicedData.set_xy`
    has resampled one of them.
    """

    def __init__(self, slice_from, slice_to):
        if not isinstance(slice_from, PathSlicedData) or not isinstance(slice_to, PathSlicedData):
            raise TypeError("PathRelativeLink requires two PathSlicedData instances")
        self._slice_from = slice_from
        self._slice_to = slice_to
        super().__init__(
            [slice_from.pixel_component_ids[-1]],
            slice_to.pixel_component_ids[-1],
            using=self._forward,
    ...
    def _forward(self, k_from):
        n_from = len(self._slice_from.x)
        n_to = len(self._slice_to.x)
...
def link_path_sliced_to_parent(data_collection, path_slice):
    """
    ...
    :func:`link_path_sliced_group`) to link the path axes of slice pairs.
    ...
    """
    non_sliced_axes = [a for a in range(path_slice.original_data.ndim)
                       if a not in path_slice.sliced_dims]
    links = []
    for slice_axis, parent_axis in enumerate(non_sliced_axes):
        link = LinkSame(path_slice.pixel_component_ids[slice_axis],
                        path_slice.original_data.pixel_component_ids[parent_axis])
        data_collection.add_link(link)
        links.append(link)
    return links


def link_path_sliced_pair_paths(data_collection, slice_a, slice_b):
    """
    Register a :class:`PathRelativeLink` between two slices' path axes.

    Caller-side assumption: the two slices trace the *same* underlying path
    ...
    order will produce slices with different ``sliced_dims`` but the same
...
def link_path_sliced_group(data_collection, *path_slices):
    """
    Wire a group of PathSlicedData instances into the link graph.

    For each slice, the non-path pixel axes are linked to the parent cube
    ...
    the slices in the group.

    Parameters
    ----------
    data_collection : :class:`~glue.core.DataCollection`
        The collection that owns all the slices and their parent cubes.
    *path_slices : :class:`PathSlicedData`
        Two or more slices to wire up. Each must already be a member of
        ``data_collection``.
    ...
    """
    if len(path_slices) < 2:
        raise ValueError("link_path_sliced_group needs at least two slices")
    links = []
    for path_slice in path_slices:
        links.extend(link_path_sliced_to_parent(data_collection, path_slice))
    for i, slice_a in enumerate(path_slices):
        for slice_b in path_slices[i + 1:]:
            links.append(link_path_sliced_pair_paths(
                data_collection, slice_a, slice_b))
    return links
