...
from .ginga_layer_artist import (ScatterLayerArtist, LayerArtistContainer,
...
class GingaClient(VizClient):
    display_data = CallbackProperty(None)
    display_attribute = CallbackProperty(None)

    def __init__(self, data, canvas=None, axes=None, artist_container=None):

        if axes is not None:
            raise ValueError("GingaClient does not accept an axes")
    ...
    def add_layer(self, layer):
        ...
        if isinstance(layer, Data):
            result = ImageLayerArtist(layer, self._canvas)
            self.artists.append(result)
            for s in layer.subsets:
                self.add_layer(s)
        elif isinstance(layer, Subset):
            result = SubsetImageLayerArtist(layer, self._canvas)
            self.artists.append(result)
