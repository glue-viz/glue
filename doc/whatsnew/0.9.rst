...
Note that the 3D viewers (provided by the glue-vispy-viewers plugin package) are
now automatically installed when installing the glueviz package.
...
no subset is selected to make a new subset (similar to other viewers), then
...
glue-vispy-viewers package (now automatically installed with glue).
