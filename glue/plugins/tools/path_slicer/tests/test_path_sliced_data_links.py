...
The realistic scenario the prototype is aiming at: an RGB-style PV
...
def _make_pv_pair_linked_cubes():
    """
    ...
    pixel i in cube_a == pixel i in cube_b). Each cube gets one PV
...
def test_generic_frb_via_links_across_linked_cubes():
    ...
    # generic FRB function can walk pv_b -> cube_b -> cube_a -> pv_a
    # entirely through the link graph, no PathSlicedData override
    # required.
    dc, cube_a, _, pv_a, pv_b = _make_pv_pair_linked_cubes()
    link_path_sliced_group(dc, pv_a, pv_b)

    bounds = [(0, 5, 6), (0, 2, 3)]
    buffer = compute_fixed_resolution_buffer(
        pv_a, bounds=bounds, target_data=pv_b, target_cid=cube_a.id['x'])
    assert buffer.shape == (6, 3)

    # Independently compute the expected values: pv_b pixel k_b ->
    # PathRelativeLink -> pv_a pixel k_a (FRB rounds), then
    # PathSlicedData._get_pix_coords truncates pv_a.x / pv_a.y when
...
def test_link_path_sliced_group_handles_three_pvs():
    # The RGB use case: three cubes, three PVs, the same path drawn in
    # each cube. After link_path_sliced_group, any PV pair can act as
    # data/target_data.
    matrix = np.eye(4)
    cubes = []
    pvs = []
    ...
    links = link_path_sliced_group(dc, *pvs)
    # 3 PVs * 1 non-path axis = 3 self-links, plus C(3, 2) = 3 path links.
    assert len(links) == 6

    # Every pair should be navigable via the link graph; spot-check by
    # asking for an FRB from each PV into each other PV and checking
    # the result has the right shape.
    bounds = [(0, 5, 6), (0, 2, 3)]
    for data_pv in pvs:
        for target_pv in pvs:
