...
requests `here <https://github.com/glue-viz/glue-wwt/issues>`__.

Experimental geospatial plugin
------------------------------

We have developed a plugin that provides a data factory which uses the
`rasterio <https://github.com/mapbox/rasterio>`_ package to read geospatial
raster data. This includes reading in the coordinate system and showing
longitude/latitude lines in image viewers, and also allows longitude/latitude
scatter data to be overplotted:
...
This plugin is experimental, and if you run into any issues or would like to
see new features, please open an issue `here
<https://github.com/glue-viz/glue-geospatial>`__.
...
histogram, scatter, or image viewers. Attributes are now set using a ``state``
attribute on the viewers. For example to set the ``x`` attribute on the scatter
plot viewer, you should set ``viewer.state.x_att`` instead of ``viewer.xatt``.
See the :ref:`programmatic` section for more details.
