...
class ModestImage(mi.AxesImage):
    ...
    def __init__(self, ax, update_while_panning=False, **kwargs):

        super(ModestImage, self).__init__(ax, **kwargs)
    ...
    def make_image(self, *args, **kwargs):
        ...
        if not self._update_while_panning and self._pressed:
            return super(ModestImage, self).make_image(*args, **kwargs)
        ...
        # TODO: avoid using floor here, instead potentially support non-ints in get_fixed_resolution_buffer
        # or just make it so extent here is in integer coordinates, so that in reality the buffer goes
        # beyond the edge of the axes
        # But this isn't ideal, because we are still limited by resolution of reference data...
        # We really need to be able to pass the exact floating point coordinates and a step
        # that corresponds to the screen resolution
        # Basically we need to have a mode for when reprojection is needed and one when it isn't.
        # Or we just always go with reprojection...
        # bounds = [(int(np.floor(ymin)), int(np.floor(ymax + 1)), 1), (int(np.floor(xmin)), int(np.floor(xmax)) + 1, 1)]
        # bounds = [(int(np.floor(ymin)), int(np.floor(ymax + 1)), 1), (int(np.floor(xmin)), int(np.floor(xmax)) + 1, 1)]
