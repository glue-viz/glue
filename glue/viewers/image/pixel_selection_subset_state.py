        ...
        try:
            ...
            if data is not self.reference_data:
                ...
                if np.array_equal(dimensions, check_dimensions):
                    pix_coord_in = [slice(0, 1) if slc.start is None else slc for slc in self.slices]
                    pix_coord_out = []
                    for idim, pix_cid in enumerate(data.pixel_component_ids):
                        if idim in correlated_dims:
                            ...
                            pix_coord_out.append(slice(coord, coord + 1))
                        else:
                            pix_coord_out.append(slice(None))

                    return data[att, tuple(pix_coord_out)]
